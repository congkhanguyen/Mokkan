...
namespace MokkAnnotator.MkaImageProcess
{
    ...
    /// </summary>
    /// 
    /// <remarks>The class encapsulates <b>HSV</b> color components.</remarks>
    /// 
    public class HSV
    {
        /// <summary>
        ...
        /// <param name="hue">Hue component.</param>
        /// <param name="saturation">Saturation component.</param>
        /// <param name="value">Luminance component.</param>
        /// 
        ...
        public static void FromRGB(RGB rgb, HSV hsv)
        {
            ...
            {
                ss = cdes * SCALE / cmax;
                if (cmax == rr) hh = (gg - bb) * SCALE / cdes;
                if (cmax == gg) hh = (bb - rr) * SCALE / cdes + 2.0 * hSCALE;
                if (cmax == bb) hh = (rr - gg) * SCALE / cdes + 4.0 * hSCALE;
            }
            else if (cdes != 0.0)
            {
                ss = cdes * SCALE / cmax;
        ...
        /// <param name="rgb">Source color in <b>RGB</b> color space.</param>
        /// 
        /// <returns>Returns <see cref="HSV"/> instance, which represents converted color value.</returns>
        /// 
        ...
        public static void ToRGB(HSV hsv, RGB rgb)
        {
            ...
            if (hsv.Hue == 6 * hGETA * (hSCALE)) hsv.Hue = 0;
            ...
            if (rr < 0) rgb.Red = 0;
            else if (rr > 255) rgb.Red = 255;
            else rgb.Red = (byte)rr;

            if (gg < 0) rgb.Green = 0;
            else if (gg > 255) rgb.Green = 255;
            else rgb.Green = (byte)gg;

            if (bb < 0) rgb.Blue = 0;
            else if (bb > 255) rgb.Blue = 255;
            else rgb.Blue = (byte)bb;
        }
    ...
    /// </summary>
    /// 
    /// <remarks>The class encapsulates <b>LAB</b> color components.</remarks>
    /// 
    public class LAB
    {
        /// <summary>
        ...
        /// </summary>
        /// 
        /// <param name="hue">Lightness component.</param>
        /// <param name="saturation">A component.</param>
        /// <param name="luminance">B component.</param>
        /// 
        public LAB(int hue, int a, int b)
        {
            this.Lightness = hue;
            this.A = a;
            this.B = b;
