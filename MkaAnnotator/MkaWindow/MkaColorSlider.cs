...
namespace MokkAnnotator.MkaWindow
{
    ...
    /// </remarks>
    /// 
    public class MkaColorSlider : System.Windows.Forms.Control
    {
        ...
        /// which is caused by user dragging the corresponding control痴 arrow (slider).</para>
        /// </remarks>
        /// 
        ...
        public MkaColorSlider()
        {
            ...
            // load arrow bitmap
            arrow = MokkAnnotator.Properties.Resources.arrow;
        ...
        private void ColorSlider_Paint(object sender, PaintEventArgs e)
        {
            ...
            g.DrawImage(arrow, x + min, y, 19, 12);
            if (doubleArrow)
                g.DrawImage(arrow, x + max, y, 19, 12);
        }

        // On mouse down
        private void ColorSlider_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
        {
            int x = (ClientRectangle.Right - width) / 2 - 9;
            int y = 3 + height;
            ...
            {
                // check X coordinate
                if ((e.X >= x + min) && (e.X < x + min + 19))
                {
                    // left arrow
                    trackMode = 1;
                    dx = e.X - min;
                }
                if ((doubleArrow) && (e.X >= x + max) && (e.X < x + max + 19))
                {
                    // right arrow
                    trackMode = 2;
                    dx = e.X - max;
